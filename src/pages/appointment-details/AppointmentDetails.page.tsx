...
const AppointmentDetails = () => {
  ...
  const { data: user } = useGetUserQuery(
    invoice?.data?.billedToUserId as string,
    {
      skip: !invoice?.data?.billedToUserId,
    }
  );
  ...
  const handleDiagnosisToMake = () => {
    ...
    onDiagnosisToMake(data)
      ...
      .catch((error) => {
        toast(
          ToastType.ERROR,
          error?.message ?? "Error updating diagnosis to make:"
        );
      });
  };
    ...
    <DashboardLayout>
      <div className="pb-16">
        <div className="flex">
          <div className="flex flex-col gap-y-3 w-[76%] pr-20">
                ...
                <Button
                  ...
                  onClick={() => {
                    if (data?.data?.chatRoomId) {
                      ...
                      dispatch(setChatroomId(data?.data?.chatRoomId as string));
                    }
                  }}
                >
                  Chat now!
            ...
            <div className="">
                ...
                <div className="flex justify-between mt-4">
                  <div className="flex gap-x-8">
                    ...
                    <div className="flex  gap-x-2">
                      <p className="text-grey4">Location Type: </p>
              ...
              {isLoading ? (
                ...
                <>
                  <div className="mt-5">
                    <p className="text-primary mb-1">Vehicle Details</p>
                    <div className="border border-primaryAccent rounded-lg p-3 flex flex-wrap space-x-14">
                      <div className="flex flex-col pr-2">
                        ...
                        <p>{data?.data?.vehicle?.model}</p>
                      </div>
                      <div className="flex flex-col pr-2 ">
                        <p className="text-gray-400 mb-2">Make</p>
                        <p>{data?.data?.vehicle?.make}</p>
                      </div>
                      <div className="flex flex-col pr-2 ">
                        <p className="text-gray-400 mb-2">Year</p>
                        <p>{data?.data?.vehicle?.year}</p>
                      </div>
                    </div>
                  </div>
                  <div className="mt-5">
                  ...
                  <div className="mt-5">
                    ...
                    <div className="border border-primaryAccent rounded-lg p-3 flex flex-wrap space-x-14">
                      ...
                      <div className="flex flex-col pr-2 ">
                        ...
                        <p>{data?.data?.serviceCenter?.locationName}</p>
                      </div>
                      <div className="flex flex-col pr-2 ">
                        <p className="text-gray-400 mb-2">Long, Lat</p>
                        <p>
                          {data?.data?.serviceCenter?.longitude},{" "}
                          {data?.data?.serviceCenter?.latitude}
                        </p>
                      </div>
                    </div>
                  </div>
          ...
          <div className="flex w-[24%] bg-white px-5 pt-5 fixed top-0 right-0 h-screen box-border drop-shadow-2xl flex-col overflow-y-auto pb-7 z-50">
            ...
            <div className="flex flex-col gap-y-6">
              <div>
                ...
                <div className="flex justify-end mt-3">
                  <Button
                    className="rounded-full text-sm float-right self-end w-auto  px-4 h-[34px]"
                    variant="primary"
              ...
              <div className="w-full">
                ...
                {!invoice?.data &&
                  data?.data?.status !== AppointmentStatus.NOT_STARTED &&
                  data?.data?.status !== AppointmentStatus.CANCELLED && (
                    <Button
