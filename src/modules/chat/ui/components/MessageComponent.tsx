...
import MessageInvoiceComponent from "./MessageInvoiceComponent";
  ...
  return (
    <div className={
      ...
      <div className={
        ...
        {reply && <p className="text-xs text-grey4 bg-white rounded-t-2xl p-2">{reply}</p>}
        {message && <p className="p-2 text-sm">{message}</p>}
        {image && <img src={image} alt="" className="w-full max-w-[330px] rounded-xl max-h-[200px] object-cover" />}
      </div>
    </div>
  )
}
