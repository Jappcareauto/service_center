...
import { SchulderCard } from "./schulderCard";
import { useNavigate } from "react-router-dom";
import { AppointmentRoutes } from "../../infra/routes/Router";

export const CalendarModal = () => {
  const { close, isOpen } = useModal({
    eventName: ModalEventKey.OPEN_CALENDAR,
  });
  ...
  return (
    <RightModal close={close} isOpen={isOpen}>
      <div className="px-4 space-y-4 py-8">
        ...
        <PrimaryButton className="my-3 rounded-full border  border-black bg-inherit text-black scale-90">
          Show Scheldule
        </PrimaryButton>
        <div className="h-full space-y-2 ">
          <SchulderCard twBorderColor="" />
          <SchulderCard
