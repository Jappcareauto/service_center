...
const AppointmentExpandDetailsView = () => {
  const { activeAppointment } = useAppointementDetail();
  return (
        ...
        <div className="mt-10 flex flex-col gap-y-6">
          ...
          <div>
            <div className="rounded-2xl bg-white border border-borderColor flex items-center justify-center w-full min-h-[190px] p-4 max-w-[355px]">
              <img src={activeAppointment.vehicle?.imageUrl} alt={activeAppointment.vehicle?.name}/>
            </div>
          ...
          <div className="flex flex-col gap-y-3 mt-5">
            ...
            <div className="flex flex-row overflow-x-auto w-full">
              {activeAppointment.vehicle?.media?.items?.map(
                (image, index) => {
                  return (
                    <img
