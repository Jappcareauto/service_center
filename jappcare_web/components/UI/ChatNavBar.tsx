...
const ChatNavBar = () => {
    ...
    const getData = async () => {
        ...
        if (resChatRoom.ok) {
            ...
            setChatRoomList(dataChat.data);
            setDiscusionsUser(dataChat.data)
        }
    }
    useEffect(() => {
                    ...
                    <div className="mt-5">
                        {
                            discusionsUser ? discusionsUser.map((item, index) => (
                                <div key={index} onClick={() => { onFadeOut(item) }}>
                                    <CardMessageUser image={""} chatRoomId={item.id} userName={item.name} lastMessage={""} time={item.updatedAt} countNewMessage={item.participantIds.length.toString()}  ></CardMessageUser>
                                </div>
